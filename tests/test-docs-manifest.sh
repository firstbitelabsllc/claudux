...
assert_contains "missing pinned heading fails validation" "$(cat /tmp/claudux-manifest-t3)" 'missing required heading "Pinned Harness Example"'
rm -rf "$TEST_DIR"
...
assert_contains "section patcher preserves pinned harness body" "$(cat /tmp/claudux-manifest-t11)" "## Pinned Harness Example"
rm -rf "$TEST_DIR"
...
assert_contains "string required field fails validation" "$(cat /tmp/claudux-manifest-t24)" "pinned-harness-example: required must be a boolean"
