...
for cmd in $advertised_cmds; do
    ...
    if grep -qx "$cmd" <<< "$handled_cmds"; then
...
if grep -qE '^\s*-q\b|--quiet' <<< "$help_output"; then
...
if grep -qi "unknown" <<< "$unknown_output"; then
...
if grep -qi "required but not installed" <<< "$unknown_output"; then
...
if grep -qi "node" <<< "$check_output"; then
...
if grep -q "IOS_TYPE=ios" <<< "$detect_result"; then
...
if grep -q "PY_TYPE=python" <<< "$detect_result"; then
...
if grep -q "GO_TYPE=go" <<< "$detect_result"; then
...
if grep -q "VIDUX_TYPE=vidux" <<< "$detect_result"; then
...
if grep -q "EMPTY_TYPE=generic" <<< "$detect_result"; then
