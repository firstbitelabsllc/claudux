...
$base_prompt"
    ...
    if [[ $claude_exit_code -eq 0 ]]; then
        ...
        if [[ "$backend" == "codex" ]]; then
            ...
            echo ""
            echo "   2. Retry with your Codex account's configured default model:"
            echo "      CLAUDUX_BACKEND=codex claudux update"
            echo "      Or set CODEX_MODEL only when you know the account supports it."
            echo ""
